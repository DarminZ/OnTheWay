...
function loadPage(page) {
	...
	var img = $('<img />');
	img.load(function() {
		var container = $('.std-docs .p'+page);
		img.css({width: container.width(), height: container.height()});
		img.appendTo($('.std-docs .p'+page));
		container.find('.loader').remove();
	});

	img.attr('src', 'pages/' +  (page-1) + '.png');
...
function updateTabs() {
	...
	$('.std-docs .tabs .left').html(left.join(''));
	$('.std-docs .tabs .right').html(right.join(''));
