...
(function (global, factory) {
  ...
  Vlist.prototype.initList = function () {
    var count =
      this.data.length < this.showItemCount
        ? this.data.length
        : this.showItemCount;
  ...
  Vlist.prototype.showLoading = function () {
    ...
    this.loadingDom.style.top =
      this.items[this.items.length - 1].top + this.itemHeight + "px";
    this.containerUL.appendChild(this.loadingDom);
  };

  //关闭loading
  Vlist.prototype.closeLoading = function () {
    if (this.loadingDom) this.containerUL.removeChild(this.loadingDom);
  };
  ...
  Vlist.prototype.diffRender = function (startIndex, startIndexNew) {
    ...
    if (moveCount >= showItemCount) {
      ...
      if (startIndex - startIndexNew > 0) {
        ...
        this.items = items
          .splice(showItemCount - moveCount, moveCount)
          .concat(items);
      } else {
  ...
  Vlist.prototype.scrollEvent = function () {
    ...
    //android手机兼容性问题
    startIndexNew =
      startIndexNew > maxStartIndex ? maxStartIndex : startIndexNew;
