...
/**
 * 给定一个数组和一个目标值，找出数组中所有满足和为目标值的4个数的组合
 */
public class Test18_4sum {
    public static void main(String[] args) {
        int[] nums1 = {1,2,1, 0, -1, 0, -2, 2};
    ...
    public static List<List<Integer>> fourSum(int[] nums, int target) {
        ...
        Arrays.sort(nums);
        if (nums[0]*4 > target && nums[nums.length-1]*4 < target) return new ArrayList<>();
        ...
        for (int i = 0;i<nums.length-3;i++){
            ...
            for (int j =i+1; j<nums.length-2;j++){
                ...
                if (nums[i] + nums[j]*3 == target && j+2 < nums.length && nums[j+2] == nums[j]){
                ...
                for (int k = j+1;k<nums.length-1;k++){
                    ...
                    if (nums[i] + nums[j] + nums[k]*2 == target && k+1 < nums.length && nums[k+1] == nums[k]){
